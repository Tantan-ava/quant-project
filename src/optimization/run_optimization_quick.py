...
def main():
    """
    主函数：执行完整的参数优化和稳健性验证流程（快速版本）
    """
    
    # 定义搜索空间（快速版本：仅36种组合）
    # 约束：panic_threshold 必须 > extreme_threshold（因为是负数，-1.0 > -2.0）
    search_space = {
        'ep_weight': [0.4, 0.5, 0.6],                     # 3种
        'osa_threshold_extreme': [-2.5],                  # 1种
        'osa_threshold_panic': [-1.5],                    # 1种
        'osa_threshold_greed': [1.5],                     # 1种
        'scalar_extreme': [1.5, 1.7],                     # 2种
        'scalar_greed': [0.5, 0.7],                       # 2种
        'top_k': [50, 100, 150]                           # 3种
    }
    # 实际组合数：3×1×1×2×2×3 = 36种
    
    print("="*60)
    print("混合频率策略自动调优与稳健性验证（快速版本）")
    print("="*60)
    ...
    if not final_candidates:
        print("\n⚠️ 没有参数组合通过稳健性检验，使用最佳候选...")
        final_candidates = [{
            'params': top_candidates[0]['params'],
            'cv_score': top_candidates[0]['cv_score'],
            'rolling': {'is_stable': False, 'cv_value': 999},
            'bootstrap': {'original_sharpe': -999}
        }]
    ...
    try:
        ...
        print(f"样本外测试失败: {e}")
        oos = {'is_positive': False, 'oos_sharpe': -999, 'oos_return': 0, 'oos_maxdd': 0}
